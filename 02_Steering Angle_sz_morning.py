...
K_OFFSET = 0.03               # offset gain (px -> deg) (0.02..0.06 typical)
...
def compute_steering(left_ab, right_ab, w, y1, y2):
    """
    Compute steering info from left/right lane lines.
    Returns dict or None if lane missing.
    """
...
def draw_hud_box(img, steer_info, fps=None):
    ...
    cv2.putText(img, "STEERING HUD", (tx, ty), font, 0.9, (0, 255, 0), 2)
    ty += line_h + 5
...
def main():
    ...
    while True:
        ...
        # 5) Steering + centerline
        steer_info = compute_steering(left_ab_s, right_ab_s, w, y1, y2)
